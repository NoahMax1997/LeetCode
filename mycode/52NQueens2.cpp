...
class Solution {
public:
    void dfs(vector<vector<string>> &ret,vector<string> &ans,set<int> &t_set0,set<int> &t_set1,set<int> &t_set2,int n,int index,int &sum){
        if(index==n){
            ret.push_back(ans);
            sum++;
            return;
        ...
        for(int i=0;i<n;i++){
            ...
            t_set0.insert(i);
            dfs(ret,ans,t_set0,t_set1,t_set2,n,index+1,sum);
            ans[index][i]='.';
            t_set0.erase(i);
    ...
    int totalNQueens(int n) {
        ...
        dfs(ret,ans,t_set0,t_set1,t_set2,n,0,sum);
        return sum;
    }
};
int main (){
